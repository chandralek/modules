...
resource "null_resource" "Install_service" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "ansible-pull -i /tmp/hosts -U https://${var.GIT_USR}:${var.GIT_PSW}@github.com/chandralek/roboshop-project.git setup.yml -t ${var.APPLICATION_NAME} -e APPLICATION_ENVIRONMENT=${var.TAGS["ENV"]} -e DNS_DOMAIN_NAME=${trim(var.HOSTED_ZONE_NAME, ".")} -e LOGSTASH=172.31.20.249"
    ]
  }
}